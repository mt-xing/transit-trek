<script lang="ts">
	...
	onMount(() => {
		const updateTeamState = async () => {
			const newTeamRes = await fetch(`/tt3/game/api/team${window.location.search}`);
			if (newTeamRes.ok) {
				try {
		...
		const updateChallenges = async () => {
			const newChallengesRes = await fetch(`/tt3/game/api/challenges${window.location.search}`);
