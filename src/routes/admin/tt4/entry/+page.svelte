<script lang="ts">
	import ChallengeEntry from '../../../../components/tt4/tt4ChallengeEntry.svelte';
	import {
		tt4ChallengeCategoryNames,
		type TT4ChallengeDefinition,
	...
	let selectedChallenge: undefined | TT4ChallengeDefinition;
	...
	<select bind:value={selectedChallenge} on:change={populateProgress}>
		...
		{#each challenges.filter( (x) => (filter ? cleanStr(x.title).indexOf(cleanStr(filter)) !== -1 : true), ) as challenge}
			<option value={challenge}
				>{tt4ChallengeCategoryNames[challenge.category]}: {challenge.title}</option
			>
		{/each}
	</select>
</p>
...
{#if selectedTeam && selectedChallenge}
	<h2>
		...
		<br />
		{tt4ChallengeCategoryNames[selectedChallenge.category]}: {selectedChallenge.title}
	</h2>
