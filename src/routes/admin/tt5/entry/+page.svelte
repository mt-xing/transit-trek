<script lang="ts">
	import ChallengeEntry from '../../../../components/tt5/tt5ChallengeEntry.svelte';
	import {
		tt5ChallengeCategoryNames,
		type TT5ChallengeDefinition,
	...
	let selectedChallenge: undefined | TT5ChallengeDefinition;
	...
	<select bind:value={selectedChallenge} on:change={populateProgress}>
		...
		{#each challenges.filter( (x) => (filter ? cleanStr(x.title).indexOf(cleanStr(filter)) !== -1 : true), ) as challenge}
			<option value={challenge}
				>{tt5ChallengeCategoryNames[challenge.category]}: {challenge.title}</option
			>
		{/each}
	</select>
</p>
...
{#if selectedTeam && selectedChallenge}
	<h2>
		...
		<br />
		{tt5ChallengeCategoryNames[selectedChallenge.category]}: {selectedChallenge.title}
	</h2>
