<script lang="ts">
	...
	$: sortedChallenges = sortTt5Challenges(
		(hideComplete
			? allChallenges?.filter((x) => !isTt5ChallengeComplete(x, team?.challengeProgress ?? {}))
			: allChallenges) ?? [],
	);
	...
	const gameStateUpdateTime = (gs?: TT5GameState) => {
		if (!gs) {
			return 10000;
		}
		switch (gs.t) {
	...
	const teamUpdateTime = (gs?: TT5GameState) => (gs?.t === 'ongoing' ? 10 * 1000 : 30 * 1000);
	const challengeUpdateTime = 60 * 1000;
	const rankUpdateTime = (gs?: TT5GameState) => (gs?.t === 'ongoing' ? 15 * 1000 : 60 * 1000);
	...
	onMount(() => {
		const updateTeamState = async () => {
			const newTeamRes = await fetch(`/tt5/game/api/team${window.location.search}`);
			if (newTeamRes.ok) {
				try {
		...
		const updateChallenges = async () => {
			const newChallengesRes = await fetch(`/tt5/game/api/challenges${window.location.search}`);
			if (newChallengesRes.ok) {
				try {
					const newChallenges = await newChallengesRes.json();
					if (newChallenges) {
						allChallenges = newChallenges as TT5PublicChallengeDefinition[];
					}
				} catch (e) {
					// eslint-disable-next-line no-console
					console.error(e);
		...
		const updateGameState = async () => {
			const newStateRes = await fetch('/tt5/game/api/game');
			if (newStateRes.ok) {
				const newState = (await newStateRes.json()) as TT5GameState | undefined;
				if (newState) {
		...
		const updateRank = async () => {
			const newRankRes = await fetch(`/tt5/game/api/rank${window.location.search}`);
			if (newRankRes.ok) {
				try {
...
{#if team && gameState}
	...
	{:else if allChallenges !== undefined}
		...
		{#each iterateTt5Categories(sortedChallenges) as category}
